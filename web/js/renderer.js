...
RenderSettings.BUFFER_SIZE =
  RenderSettings.CHUNKS_IN_BUFFER * RenderSettings.CHUNK_SIZE;
...
const renderer = {
  ...
  Buffer: {
    ...
    getFor(worldX, worldY, create = true) {
      let i = floor(
        (worldX * RenderSettings.BLOCK_RESOLUTION) / RenderSettings.BUFFER_SIZE
      );

      let j = floor(
        (worldY * RenderSettings.BLOCK_RESOLUTION) / RenderSettings.BUFFER_SIZE
      );
    ...
    isVisible(buffer) {
      ...
      buffer.canvasX =
        buffer.i * RenderSettings.BUFFER_SIZE * renderer.ViewPort.scale +
        renderer.ViewPort.offsetX +
        renderer.ViewPort.tempOffsetX;

      buffer.canvasY =
        buffer.j * RenderSettings.BUFFER_SIZE * renderer.ViewPort.scale +
        renderer.ViewPort.offsetY +
        renderer.ViewPort.tempOffsetY;
  ...
  ViewPort: {
    ...
    zoomAnchor: [0, 0], // TODO: Move appropriate sections of code to this object
    ...
    canvasToWorld(canvasX, canvasY) {
      return [
        floor(
          ((canvasX - renderer.ViewPort.offsetX) *
            (1 / renderer.ViewPort.scale)) /
            RenderSettings.BLOCK_RESOLUTION
        ),

        floor(
          ((canvasY - renderer.ViewPort.offsetY) *
            (1 / renderer.ViewPort.scale)) /
            RenderSettings.BLOCK_RESOLUTION
        ),
    ...
    worldToCanvas(worldX, worldZ, offset = true) {
      return [
        worldX * renderer.ViewPort.scale +
          (offset
            ? renderer.ViewPort.offsetX + renderer.ViewPort.tempOffsetX
            : 0),

        worldZ * renderer.ViewPort.scale +
          (offset
    ...
    setScale(value, moveTo = null) {
      renderer.ViewPort.scale = min(
        renderer.ViewPort.scaleMax,

        max(renderer.ViewPort.scaleMin, round(parseFloat(value), 2))
      );
  ...
  setup: () => {
    renderer.BlockPainter = new FlatColorBlockPainter(); // renderer.BlockPainter = new TexturedBlockPainter();

    renderer.ViewPort.setOffsets(width / 2, height / 2); // TODO: move to preload
  ...
  _processChunkRenderQueue: async (chunk = null, force = false) => {
    ...
    if (chunk) {
      ...
      chunksToRender = renderer.chunkRenderQueue.splice(
        0,

        RenderSettings.CHUNK_RENDER_RATE
      );
    }
    ...
    for (var i = chunksToRender.length - 1; i >= 0; i--) {
      ...
      var buffer = renderer.Buffer.getFor(chunk.x << 4, chunk.z << 4); // If buffer is not visible in the viewport, don't bother rendering the chunk // put it the render queue, unless force is set to true
      ...
      let i = buffer.i < 0 ? -1 : 1; // Quadrant I  & IV

      let j = buffer.j < 0 ? -1 : 1;

      var chunkBufferX =
        Math.abs((chunk.x + (i < 0 ? 1 : 0)) % 16 << 4) *
        RenderSettings.BLOCK_RESOLUTION;

      var chunkBufferZ =
        Math.abs((chunk.z + (j < 0 ? 1 : 0)) % 16 << 4) *
        RenderSettings.BLOCK_RESOLUTION; // Left
      ...
      for (let x = 0; x < 16; x++) {
        for (let z = 0; z < 16; z++) {
          ...
          if (
            ...
            console.error(
              `Outside buffer rendering on x axis detected. (Target: ${
                chunkBufferX + x
              }) (Buffer: ${buffer.i}, ${buffer.j}) (Chunk: ${chunk.x}, ${
                chunk.z
              })`
            );
          }
          ...
          if (
            ...
            console.error(
              `Outside buffer rendering on y axis detected. (Target: ${
                chunkBufferZ + z
              }) (Buffer: ${buffer.i}, ${buffer.j}) (Chunk: ${chunk.x}, ${
                chunk.z
              })`
  ...
  renderGridOverlay: () => {
    ...
    var xOff =
      (renderer.ViewPort.offsetX + renderer.ViewPort.tempOffsetX) % chunkSize;

    var yOff =
      (renderer.ViewPort.offsetY + renderer.ViewPort.tempOffsetY) % chunkSize;
  ...
  renderBufferOutlines: () => {
    ...
    Object.values(renderer.Buffer.loaded).forEach((buffer) => {
      ...
      rect(
        ...
        RenderSettings.BUFFER_SIZE * renderer.ViewPort.scale,
  ...
  renderFrameRateOnCanvas: () => {
    ...
    noStroke(); // add little background
  ...
  drawPlayers: () => {
    ...
    players.forEach((player) => {
      ...
      let coords = renderer.ViewPort.worldToCanvas(
        player.position.x,
  ...
  renderMouseCoordinates: () => {
    ...
    let padding = 2; // World Coordinates
    ...
    text(txt, mouseX + 20, mouseY - 20); // Chunk Coordinates
    ...
    text(txt, mouseX + 20, mouseY - 8); // Block ID
