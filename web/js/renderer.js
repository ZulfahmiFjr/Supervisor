...
const renderer = {
    ...
    Buffer: {
        ...
        isVisible(buffer) {
            ...
            buffer.visible =
                buffer.canvasRY > 0 && 0 < buffer.canvasRX && buffer.canvasY < height && buffer.canvasX < width;
    ...
    ViewPort: {
        ...
        canvasToWorld(canvasX, canvasY) {
            return [
                floor(
                    ((canvasX - renderer.ViewPort.offsetX) * (1 / renderer.ViewPort.scale)) /
                        RenderSettings.BLOCK_RESOLUTION
                ),

                floor(
                    ((canvasY - renderer.ViewPort.offsetY) * (1 / renderer.ViewPort.scale)) /
                        RenderSettings.BLOCK_RESOLUTION
                ),
        ...
        worldToCanvas(worldX, worldZ, offset = true) {
            return [
                worldX * renderer.ViewPort.scale +
                    (offset ? renderer.ViewPort.offsetX + renderer.ViewPort.tempOffsetX : 0),

                worldZ * renderer.ViewPort.scale +
                    (offset ? renderer.ViewPort.offsetY + renderer.ViewPort.tempOffsetY : 0),
            ];
        },
    ...
    _processChunkRenderQueue: async (chunk = null, force = false) => {
        ...
        for (var i = chunksToRender.length - 1; i >= 0; i--) {
            ...
            var chunkBufferZ = Math.abs((chunk.z + (j < 0 ? 1 : 0)) % 16 << 4) * RenderSettings.BLOCK_RESOLUTION; // Left
            ...
            for (let x = 0; x < 16; x++) {
                for (let z = 0; z < 16; z++) {
                    ...
                    if (chunkBufferX + x * renderer.Buffer.BLOCK_RESOLUTION >= buffer.width) {
                        console.error(
                            `Outside buffer rendering on x axis detected. (Target: ${chunkBufferX + x}) (Buffer: ${
                                buffer.i
                            }, ${buffer.j}) (Chunk: ${chunk.x}, ${chunk.z})`
                        );
                    }

                    if (chunkBufferZ + z * renderer.Buffer.BLOCK_RESOLUTION >= buffer.height) {
                        console.error(
                            `Outside buffer rendering on y axis detected. (Target: ${chunkBufferZ + z}) (Buffer: ${
                                buffer.i
                            }, ${buffer.j}) (Chunk: ${chunk.x}, ${chunk.z})`
                        );
                    }
