...
const UI = {
  ...
  setup: () => {
    ...
    UI.statsBuffersRendered = document.getElementById(
      "stats-buffers-rendered-value"
    );
    ...
    connectPocketCore(
      ...
      () => {
        document
          .getElementById("connection-light")
          .classList.remove("disconnected-light");
        document
          .getElementById("connection-light")
          .classList.add("connected-light");
      },
      () => {
        document
          .getElementById("connection-light")
          .classList.remove("connected-light");
        document
          .getElementById("connection-light")
          .classList.add("disconnected-light");
      }
    );
  },
  ...
  update: () => {
    ...
    if (frameCount % 20 === 0) {
      let toShow =
        UI.fpsTracked.reduce((acc, curr) => acc + curr) / UI.fpsTracked.length;
      ...
      UI.statsBuffersCount.innerHTML = Object.values(
        renderer.Buffer.loaded
      ).length;
      UI.statsBuffersRendered.innerHTML =
        renderer.Buffer.cachedVisibility.length;
    ...
    players.forEach((player) => {
      let coord = renderer.ViewPort.worldToCanvas(
        player.position.x,
        player.position.z,
        true
      );
  ...
  renderPlayerCard: (x, y, players) => {
    ...
    players.forEach((player, i) => {
      ...
      rect(
        0,
        0,
        containerHeight + textWidth(name) + padding,
        faceSize + padding * 2
      );
  ...
  mouseDragged: () => {
    ...
    if (
      (mouseX <= width && mouseX >= 0 && mouseY <= height && mouseY >= 0) ===
      false
    ) {
      UI.mouseReleased();
      return;
  ...
  handleMovement: () => {
    ...
    if (keyIsDown(RIGHT_ARROW)) {
      renderer.ViewPort.setOffsetX(
        renderer.ViewPort.offsetX - 10 * renderer.ViewPort.scale
      );
    }
    if (keyIsDown(UP_ARROW)) {
      renderer.ViewPort.setOffsetY(
        renderer.ViewPort.offsetY + 10 * renderer.ViewPort.scale
      );
    }
    ...
    // Press Z to Zoom In
    if (keyIsDown(90)) {
      renderer.ViewPort.setScale(
        (renderer.ViewPort.scale += 0.1),
        renderer.ViewPort.middleBlock
      );
    }
    // Press X to Zoom Out
    if (keyIsDown(88)) {
